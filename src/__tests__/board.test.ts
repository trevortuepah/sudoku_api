...
describe('validateMove', () => {
  describe('returns invalid', () => {
    test('if you try to play in a filled spot', () => {
      expect(() => validateMove(testBoard, 2, [0,1])).toThrow("This tile already has a value. Erase it first if you'd like to update it");
    });
    test('if you try to play a value in a row that already contains that value', () => {
      expect(() => validateMove(testBoard, 7, [0,0])).toThrow('This value already exists in this row');
    });
    test('if you try to play a value in a column that already contains that value', () => {
      expect(() => validateMove(testBoard, 3, [8,0])).toThrow('This value already exists in this column');
    });
    test('if you try to play a value in a column that already contains that value', () => {
      expect(() => validateMove(testBoard, 3, [8,0])).toThrow('This value already exists in this column');
    });
    test('if you try to play a value in a 3x3 box that already contains that value', () => {
      expect(() => validateMove(testBoard, 7, [1,0])).toThrow('This value already exists in this box');
    });
  });
